...
Запрашивает JSON-формат подписки у провайдеров, которые поддерживают экспорт полной Xray-конфигурации. Используйте это, если провайдер отдаёт balancer/grouped конфиги только в JSON-формате.
...
Кастомный заголовок `User-Agent` для запросов подписки. Если задан, переопределяет User-Agent, который Xray Checker выбирает по умолчанию.
...
Дополнительные заголовки запросов подписки в формате `Key:Value`. CLI-флаг можно указать несколько раз; для переменной окружения используйте формат списка, принятый в вашем способе запуска.
...
Разрешает показывать детали серверов в public-режиме, если дашборд уже защищён доверенным внешним auth-прокси. Оставляйте выключенным для напрямую публичной статус-страницы.
